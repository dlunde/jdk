...
PhiNode *ConnectionGraph::split_memory_phi(PhiNode *orig_phi, int alias_idx, GrowableArray<PhiNode *> &orig_phi_worklist, uint rec_depth) {
  ...
  while(!finished) {
    while (idx < phi->req()) {
      Node *mem = find_inst_mem(phi->in(idx), alias_idx,
                                orig_phi_worklist, rec_depth + 1);
...
//
#define FIND_INST_MEM_RECURSION_DEPTH_LIMIT 300
Node* ConnectionGraph::find_inst_mem(Node *orig_mem, int alias_idx, GrowableArray<PhiNode *>  &orig_phis, uint rec_depth) {
  if (rec_depth > FIND_INST_MEM_RECURSION_DEPTH_LIMIT ) {
    _compile->record_failure(_invocation > 0 ?
                             C2Compiler::retry_no_iterative_escape_analysis() :
                             C2Compiler::retry_no_escape_analysis());
    return nullptr;
  }
  if (orig_mem == nullptr) {
    return orig_mem;
