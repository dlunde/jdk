...
Block* PhaseCFG::insert_anti_dependences(Block* LCA, Node* load, bool verify) {
  ...
  // The relevant stores "nearby" the load consist of a tree rooted at
  // initial_mem with internal nodes of type MergeMem. Therefore, the
  // branches visited by the worklist are of this form:
  //    initial_mem -> (MergeMem ->)* Memory state modifying node
  // Memory state modifying nodes include Store and Phi nodes and any node for
  // which needs_anti_dependence_check() returns false. The anti-dependence
  // constraints apply only to the fringe of this tree.
  //
  // In some cases, there are other relevant initial memory states besides
  // initial_mem. In such cases, we are in fact dealing with multiple trees and
  // their fringes.
  ...
  if (load->in(0) != nullptr) {
    ...
    // from the early block to the initial memory block. If we in a block find
    // memory Phi(s) that can alias initial_mem, these are also potential
    // initial memory states and there may be further required anti-dependences
    // due to them.
    assert(initial_mem_block->dominates(early), "invariant");
    Block* b = early;
    // Stop searching when we step past the initial memory block (b ==
    // initial_mem_block->_idom). Always terminates because the root block
    // strictly dominates initial_mem_block.
    while (b != initial_mem_block->_idom) {
      ...
      // We need to traverse all memory Phi nodes in the block. We may not have
      ...
      for (uint i = 0; i < b->number_of_nodes(); ++i) {
        ...
        if (n->is_memory_phi() && C->can_alias(n->adr_type(), load_alias_idx) && n != initial_mem) {
          // We have found a potentially relevant Phi initial memory state
