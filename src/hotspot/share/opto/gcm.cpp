...
static Block* raise_LCA_above_marks_new(Block* LCA, VectorSet& LCA_marks_new, Block* early, const PhaseCFG* cfg) {
  Block_List worklist;
  VectorSet visited;
  worklist.push(LCA);
  while (worklist.size() > 0) {
    ...
    // Test and set the visited bit.
    if (visited.test(mid->_pre_order))  continue;  // already visited

    // Don't process the current LCA, otherwise the search may terminate early
    if (mid != LCA && LCA_marks_new.test(mid->_pre_order)) {
      // Raise the LCA.
      LCA = mid->dom_lca(LCA);
      if (LCA == early)  break;   // stop searching everywhere
      assert(early->dominates(LCA), "early is high enough");
    ...
    visited.set(mid->_pre_order);
  }
  return LCA;
}

/*
 * This is the version walking the dominator tree between initial_mem_block and
 * early, collecting equivalent memory nodes.
 */
Block* PhaseCFG::insert_anti_dependences_new_dominator(Block* LCA, Node* load, Verifier& verifier, bool verify) {
  /* ResourceMark rm; */
...
/* #endif */
      ...
      for (uint j = PhiNode::Input, jmax = use_mem_state->req(); j < jmax; j++) {
        if (use_mem_state->in(j) == def_mem_state) {   // Found matching input?
          ...
          if (pred_block != early) {
            ...
            verifier.LCA_marks.set(pred_block->_pre_order);
  ...
  if (must_raise_LCA)
    LCA = raise_LCA_above_marks_new(LCA, verifier.LCA_marks, early, this);
  if (LCA == early)  return LCA;
  ...
  if (verifier.LCA_marks.test(LCA->_pre_order)) {
...
class DefUseMemStatesQueueNew : public StackObj {
private:
  class DefUsePair : public StackObj {
  private:
    Node* _def; // memory state
  ...
  GrowableArray<DefUsePair> _queue;
  GrowableArray<Node*> _visited_defs; // visited defs

  bool already_enqueued(Node* use_mem) const {
    for (int j = _queue.length()-1; j >= 0; j--) {
      if (_queue.at(j).use() == use_mem) {
        return true;
      }
    }
    return false;
...
public:
  DefUseMemStatesQueueNew(ResourceArea* area) {
  }
  ...
  void push(Node* def_mem_state, Node* use_mem_state) {
    if (use_mem_state->is_Phi()) {
      ...
      // Be sure we don't get into combinatorial problems.
      if(already_enqueued(use_mem_state)) {
        return; // already on work list; do not repeat
      }
    }
  ...
  bool has_visited_def(Node* def_mem_state) {
    return _visited_defs.contains(def_mem_state);
  }

  void mark_visited_def(Node* def_mem_state) {
    assert(!_visited_defs.contains(def_mem_state), "invariant");
    _visited_defs.append(def_mem_state);
  }
...
/*
 * This is the version walking the memory graph upwards to find all potential
 * defining memory nodes, and then downwards through Phis if initial_mem lives
 * on all Phi inputs (but we also still raise the LCA above Phi inputs).
 */
Block* PhaseCFG::insert_anti_dependences_new_memory_graph(Block* LCA, Node* load, Verifier& verifier, bool verify) {
  /* assert(load->needs_anti_dependence_check(), "must be a load of some sort"); */
  /* assert(LCA != nullptr, ""); */
  DEBUG_ONLY(Block* LCA_orig = LCA);
...
/* #endif */
  ...
  ResourceArea* area = Thread::current()->resource_area();
  DefUseMemStatesQueueNew worklist_def_use_mem_states(area); // prior memory state to store and possible-def to explore
  Node_List non_early_stores(area); // all relevant stores outside of early
  bool must_raise_LCA = false;
  ...
  for (int i = 0; i < worklist_def_use_mem_states.length(); i++) {
    Node* n = worklist_def_use_mem_states.at(i).use();

    if (n->is_Phi()) {
      for (uint i = PhiNode::Input, imax = n->req(); i < imax; i++) {
        worklist_def_use_mem_states.push(nullptr, n->in(i));
      }
      ...
      Node* p = mm->memory_at(load_alias_idx);
      if (p != mm->base_memory()) {
        worklist_def_use_mem_states.push(nullptr, p);
      }
    }
  }
  while (worklist_def_use_mem_states.is_nonempty()) {
...
bool PhaseCFG::needs_anti_dependence(Node* use_mem_state, Node* load, int load_alias_idx, uint op) {
...
/*
 * Currently the same as new_dominator with some additional assertion codes.
 * Also, I added needs_anti_dependence and check for more things before adding
 * to worklist.
 */
Block* PhaseCFG::insert_anti_dependences_new_experimental(Block* LCA, Node* load, Verifier& verifier, bool verify) {
  /* ResourceMark rm; */
  /* assert(load->needs_anti_dependence_check(), "must be a load of some sort"); */
  /* assert(LCA != nullptr, ""); */
  DEBUG_ONLY(Block* LCA_orig = LCA);
...
/* #endif */
  ...
  /* assert(initial_mem_block->dominates(early), "invariant"); */
#ifdef ASSERT
  // Check that initial_mem is the earliest possible definition of the load's
  // memory

  // Collect all initial_mem candidates
  Node_List worklist;
  VectorSet visited;
  Node_List candidates;
  worklist.push(initial_mem);
  while (worklist.size() > 0) {
    Node* n = worklist.pop();
    visited.set(n->_idx);
    Block* n_block = get_block_for_node(n);
    if (n_block != nullptr && n_block != initial_mem_block && n_block->dominates(initial_mem_block)) {
      candidates.push(n);
    }
    ...
    if (n->is_Phi()) {
      ...
      MergeMemNode* mm = n->as_MergeMem();
      Node* p = mm->memory_at(load_alias_idx);
      if (p != nullptr && !visited.test(p->_idx)) {
        worklist.push(p);
      }
    }
  }

  VectorSet visited_from;
  static int count = 0;
  static int total = 0;
  ++total;
  bool found_initial_mem = false;
  while(candidates.size() > 0) {
    Node* initial_mem_candidate = candidates.pop();

    worklist.clear();
    visited.reset();
    visited_from.reset();
    worklist.push(initial_mem_candidate);
    while (worklist.size() > 0) {
      Node* n = worklist.pop();
      bool add_children = false;
      if (n == initial_mem_candidate) {
        add_children = true;
      }
      if (n->is_MergeMem()) {
        add_children = true;
        visited.set(n->_idx);
        if (n == initial_mem) {
          found_initial_mem = true;
        }
      } else if (n->is_Phi()) {
        add_children = true;
        for (uint i = PhiNode::Input, imax = n->req(); i < imax; i++) {
          Node* in = n->in(i);
          if(!visited_from.test(in->_idx)) {
            add_children = false;
            break;
          }
        }
      ...
      if (add_children) {
        visited_from.set(n->_idx);
        for (DUIterator_Fast imax, i = n->fast_outs(imax); i < imax; i++) {
          Node* out = n->fast_out(i);
          if (visited.test(out->_idx) || out->needs_anti_dependence_check()) {
            continue;
          }
          worklist.push(out);
        }
  ...
  if (found_initial_mem) {
    ++count;
    tty->print_cr("%d / %d", count, total);
  }
...
/* #endif */
    ...
    if (!needs_anti_dependence(use_mem_state, load, load_alias_idx, op)) {
      continue;
    }
...
void verify_debug(Node* load, Verifier& old, Verifier& new1, Verifier& new2, Verifier& new3) {
  if (UseNewCode && Compile::current()->is_method_compilation()) {
    static int diffs = 0;
    static int total = 0;
    bool diff = false;
    if (old.LCA != new1.LCA || new1.LCA != new2.LCA || new2.LCA != new3.LCA) {
      diff = true;
      tty->print_cr("LCAs differ. Old: %u, New1: %u, New2: %u, New3: %u",
          old.LCA->_pre_order, new1.LCA->_pre_order,
          new2.LCA->_pre_order, new3.LCA->_pre_order);
      tty->print("Old marks: "); old.LCA_marks.dump(); tty->cr();
      tty->print("New1 marks: "); new1.LCA_marks.dump(); tty->cr();
      tty->print("New2 marks: "); new2.LCA_marks.dump(); tty->cr();
      tty->print("New3 marks: "); new3.LCA_marks.dump(); tty->cr();
      tty->print_cr("LCA dom depth: %d", old.LCA->_dom_depth);
      tty->print_cr("LCA_new dom depth: %d", new1.LCA->_dom_depth);
      tty->print_cr("LCA_new2 dom depth: %d", new2.LCA->_dom_depth);
      tty->print_cr("LCA_new3 dom depth: %d", new3.LCA->_dom_depth);
    }
    if (!(old.anti_dependences == new1.anti_dependences) ||
        !(new1.anti_dependences == new2.anti_dependences) ||
        !(new2.anti_dependences == new3.anti_dependences)) {
      diff = true;
      tty->print_cr("Anti dependence edges differ.");
      tty->print("Old: "); old.anti_dependences.dump(); tty->cr();
      tty->print("New1: "); new1.anti_dependences.dump(); tty->cr();
      tty->print("New2: "); new2.anti_dependences.dump(); tty->cr();
      tty->print("New3: "); new3.anti_dependences.dump(); tty->cr();
    }
    if(!(new1.initial_mems == new2.initial_mems)) {
      diff = true;
      tty->print_cr("Initial mems differ");
      tty->print("New1: "); new1.initial_mems.dump(); tty->cr();
      tty->print("New2: "); new2.initial_mems.dump(); tty->cr();
    }
    total++;
    if (diff) {
      diffs++;
      tty->print_cr("Load: %u", load->_idx);
...
Block* PhaseCFG::insert_anti_dependences(Block* LCA, Node* load, bool verify) {
  ...
  if (UseNewCode || UseNewCode2) {
    new1.LCA = insert_anti_dependences_new_dominator(LCA, load, new1, verify);
    new2.LCA = insert_anti_dependences_new_memory_graph(LCA, load, new2, verify);
    new3.LCA = insert_anti_dependences_new_experimental(LCA, load, new3, verify);
  }
  if (UseNewCode2) {
    return new3.LCA;
  }
  ...
  if (!C->alias_type(load_alias_idx)->is_rewritable()) {
    ...
    // which 'load' must witness.
    old.LCA = LCA;
    verify_debug(load, old, new1, new2, new3);
    return LCA;
  }
  ...
  while (worklist_def_use_mem_states.is_nonempty()) {
    ...
    if (use_mem_state->is_Phi()) {
      ...
      for (uint j = PhiNode::Input, jmax = use_mem_state->req(); j < jmax; j++) {
        if (use_mem_state->in(j) == def_mem_state) {   // Found matching input?
          ...
          if (pred_block != early) {
            ...
            // since the load will be forced into a block preceding the Phi.
            pred_block->set_raise_LCA_mark(load_index);
            old.LCA_marks.set(pred_block->_pre_order);
            assert(!LCA_orig->dominates(pred_block) ||
                   early->dominates(pred_block), "early is high enough");
            must_raise_LCA = true;
          } else {
      ...
      if (LCA != early && !unrelated_load_in_store_null_block(use_mem_state, load)) {
        store_block->set_raise_LCA_mark(load_index);
        old.LCA_marks.set(store_block->_pre_order);
        must_raise_LCA = true;
        non_early_stores.push(use_mem_state);
      }
    } else {
      ...
      if (verify) {
        ...
        use_mem_state->add_prec(load);
        old.anti_dependences.set(use_mem_state->_idx);
      }
      LCA = early;
      // This turns off the process of gathering non_early_stores.
    }
    ...
    while (non_early_stores.size() > 0) {
      ...
      if (store_block == LCA) {
        ...
        if (verify) {
          ...
          store->add_prec(load);
          old.anti_dependences.set(store->_idx);
        }
      } else {
        assert(store_block->raise_LCA_mark() == load_index, "block was marked");
        // Any other stores we found must be either inside the new LCA
