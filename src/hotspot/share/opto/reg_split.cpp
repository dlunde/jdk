...
uint PhaseChaitin::Split_new(uint maxlrg, ResourceArea* split_arena) {
  ...
  for( bidx = 0; bidx < _cfg.number_of_blocks(); bidx++ ) {
    ...
    for( slidx = 0; slidx < spill_cnt; slidx++ ) {
      ...
      if( needs_phi || has_phi ) {
        ...
        // As a baseline, use a weighted average of the predecessor's frequency
        // and UP/DOWN status.
        UPblock[slidx] = u_weight > 0.0;
...
uint PhaseChaitin::Split(uint maxlrg, ResourceArea* split_arena) {
  if (UseNewCode) {
    return Split_new(maxlrg, split_arena);
  }
